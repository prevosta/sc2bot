...
class RampWallBot(BotAI):
    ...
    async def on_step(self, iteration):
        ...
        for refinery in self.gas_buildings:
            ...
            if refinery.assigned_harvesters > refinery.ideal_harvesters:
                worker: Units = self.workers.gathering.closer_than(4, refinery).random
                if worker:
                    worker.gather(self.mineral_field.closest_to(cc))
        ...
        # Send 3 worker to repair domaged structure
        for structure in self.structures:
            if structure.health < structure.health_max:
                workers = self.workers.gathering
                if workers:
                    for worker in workers[:3]:  # Send 3 workers
        ...
        if depots.ready and self.can_afford(UnitTypeId.BARRACKS) and self.already_pending(UnitTypeId.BARRACKS) == 0:
            if self.structures(UnitTypeId.BARRACKS).amount + self.already_pending(UnitTypeId.BARRACKS) < self.n_barracks:
                workers = self.workers.gathering
        ...
        # 3-Build refineries
        n_refineries = len(self.structures(UnitTypeId.COMMANDCENTER)) * 2
        n_refineries = min(n_refineries, len(self.structures(UnitTypeId.BARRACKS)) + len(self.structures(UnitTypeId.FACTORY)))
        if self.structures(UnitTypeId.BARRACKS) and self.gas_buildings.amount < n_refineries:
        ...
        # 4-Build (2) Reapers
        if self.structures(UnitTypeId.BARRACKS).ready and self.units(UnitTypeId.REAPER).amount < self.n_reapers:
            for barracks in self.structures(UnitTypeId.BARRACKS).ready.idle:
                if self.can_afford(UnitTypeId.REAPER) and barracks.is_idle:
                    barracks.train(UnitTypeId.REAPER)

        # 5-Build (2) Marines
        if self.structures(UnitTypeId.BARRACKS).ready and self.units(UnitTypeId.MARINE).amount < self.n_marines:
            for barracks in self.structures(UnitTypeId.BARRACKS).ready.idle:
                if self.can_afford(UnitTypeId.MARINE) and barracks.is_idle:
                    barracks.train(UnitTypeId.MARINE)
        
        # 6-Build Factory
        if self.structures(UnitTypeId.BARRACKS).amount > 0 and self.can_afford(UnitTypeId.FACTORY):
            if self.structures(UnitTypeId.FACTORY).amount + self.already_pending(UnitTypeId.FACTORY) < self.n_factories:
                workers = self.workers.gathering
                if workers:
                    worker: Unit = workers.random
                    worker.build(UnitTypeId.FACTORY, cc.position.towards(self.game_info.map_center, 8))
    ...
    def draw_placement_grid(self):
        ...
        for (b, a), value in np.ndenumerate(self.game_info.placement_grid.data_numpy):
            ...
            h2 = self.get_terrain_z_height(p)
            pos = Point3((p.x, p.y, h2))
            p0 = Point3((pos.x - 0.5, pos.y - 0.5, pos.z + 0.1)) + Point2((0.5, 0.5))
            p1 = Point3((pos.x + 0.5, pos.y + 0.5, pos.z - 0.1)) + Point2((0.5, 0.5))
            # logger.info(f"Drawing {p0} to {p1}")
            color = Point3((0, 255, 0))
            self.client.debug_box_out(p0, p1, color=color)
