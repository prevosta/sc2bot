...
class BuildManager:
    def __init__(self, bot: BotAI):
        self.bot = bot
        self.build_tasks = {}  # {structure_type: {"pos": Point2, "scv_tag": int, "started": bool}}

    async def smart_build(self, structure_type, near, **kwargs):
        # If already building or pending, skip
        if self.bot.structures(structure_type) or self.bot.already_pending(structure_type):
            self.build_tasks.pop(structure_type, None)
            return

        # Get or set build task
        task = self.build_tasks.get(structure_type)
        if not task:
            pos = await self.bot.find_placement(structure_type, near)
            if not pos:
                return
            # Find closest available SCV
            scv = self.bot.workers.idle | self.bot.workers.gathering
            if not scv:
                return
            scv = scv.closest_to(pos)
            self.build_tasks[structure_type] = {"pos": pos, "scv_tag": scv.tag, "started": False}
            task = self.build_tasks[structure_type]

        pos = task["pos"]
        scv = self.bot.workers.find_by_tag(task["scv_tag"])
        if not scv:
            self.build_tasks.pop(structure_type, None)
            return

        # Estimate time to reach position
        dist = scv.position.distance_to(pos)
        speed = 3.15  # SCV speed
        frames_to_arrive = dist / speed * 22.4  # frames
        minerals_needed = self.bot.calculate_cost(structure_type).minerals

        # Estimate mineral income per second (simple heuristic)
        mineral_income_per_sec = max(1, self.bot.workers.amount * 0.7)
        time_to_afford = max(0, (minerals_needed - self.bot.minerals) / mineral_income_per_sec)
        frames_to_afford = time_to_afford * 22.4
        print(f"Build {structure_type}{pos} - Time to afford: {time_to_afford:.1f}s")

        # If can afford, build
        if self.bot.can_afford(structure_type):
            await self.bot.build(structure_type, near=pos, **kwargs)
            self.build_tasks.pop(structure_type, None)
            return

        # If not close enough, send SCV to position
        if not task["started"]:
            if frames_to_arrive > frames_to_afford - 22.4:  # 1s before
                scv.move(pos)
                return
            else:
                task["started"] = True
